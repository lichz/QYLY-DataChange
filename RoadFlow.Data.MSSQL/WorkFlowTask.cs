...
//                WHERE a.ID=(SELECT TOP 1 ID FROM WorkFlowTask WHERE FlowID=a.FlowID AND GroupID=a.GroupID ORDER BY Sort DESC)");
            ...
            //if (status != 0) {
            //    if (status == 1) {
            //        where.Append(" AND a.Status IN(0,1,5)");
            //    } else if (status == 2) {
            //        where.Append(" AND a.Status IN(2,3,4)");
            //    }
            //}

            //if (flowID != null && flowID.Length > 0) {
            //    where.Append(string.Format(" AND a.FlowID IN({0})", RoadFlow.Utility.Tools.GetSqlInString(flowID)));
            //}
            //if (senderID != null && senderID.Length > 0) {
            //    if (senderID.Length == 1) {
            //        where.Append(" AND a.SenderID=@SenderID");
            //        parList.Add(new SqlParameter("@SenderID", SqlDbType.UniqueIdentifier) { Value = senderID[0] });
            //    } else {
            //        where.Append(string.Format(" AND a.SenderID IN({0})", RoadFlow.Utility.Tools.GetSqlInString(senderID)));
            //    }
            //}
            //if (receiveID != null && receiveID.Length > 0) {
            //    if (senderID.Length == 1) {
            //        where.Append(" AND a.ReceiveID=@ReceiveID");
            //        parList.Add(new SqlParameter("@ReceiveID", SqlDbType.UniqueIdentifier) { Value = receiveID[0] });
            //    } else {
            //        where.Append(string.Format(" AND a.ReceiveID IN({0})", RoadFlow.Utility.Tools.GetSqlInString(receiveID)));
            //    }
            //}
            //if (!title.IsNullOrEmpty()) {
            //    where.Append(" AND CHARINDEX(@Title,a.Title)>0");
            //    parList.Add(new SqlParameter("@Title", SqlDbType.NVarChar, 2000) { Value = title });
            //}
            //if (flowid.IsGuid()) {
            ...
            if (status != 0) {
                if (status == 1) {
                    where.Append(" AND Status IN(0,1,5)");
                } else if (status == 2) {
                    //where.Append(" AND Status IN(2,3,4)");
                    where.Append(" AND Status IN(2,3)");
                }
            }
            if (flowID != null && flowID.Length > 0) {
                where.Append(string.Format(" AND FlowID IN({0})", RoadFlow.Utility.Tools.GetSqlInString(flowID)));
            }
            if (senderID != null && senderID.Length > 0) {
                if (senderID.Length == 1) {
                    where.Append(" AND SenderID=@SenderID");
                    parList.Add(new SqlParameter("@SenderID", SqlDbType.UniqueIdentifier) { Value = senderID[0] });
                } else {
                    where.Append(string.Format(" AND SenderID IN({0})", RoadFlow.Utility.Tools.GetSqlInString(senderID)));
                }
            }
            if (receiveID != null && receiveID.Length > 0) {
                if (senderID.Length == 1) {
                    where.Append(" AND ReceiveID=@ReceiveID");
                    parList.Add(new SqlParameter("@ReceiveID", SqlDbType.UniqueIdentifier) { Value = receiveID[0] });
                } else {
                    where.Append(string.Format(" AND ReceiveID IN({0})", RoadFlow.Utility.Tools.GetSqlInString(receiveID)));
                }
            }
            if (!title.IsNullOrEmpty()) {
        ...
        public int Completed(Guid taskID, string comment = "", bool isSign = false, int status = 2, string note = "") {
            ...
            SqlParameter[] parameters = new SqlParameter[]{
                ...
                new SqlParameter("@CompletedTime1", SqlDbType.DateTime){ Value = DateTime.Now },
