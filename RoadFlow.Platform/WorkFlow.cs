    ...
    public class WorkFlow
    {
        ...
        public string InstallFlow(string jsonString, bool isMvc = true)
        {
            ...
            else
            {
                ...
                {
                    //using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope(TransactionScopeOption.Required, new TimeSpan(6000000000)))
        ...
        /// <param name="flowID">流程ID</param>
        /// <returns></returns>
        public RoadFlow.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(string flowID, bool cache = false)
        {
            Guid fid;
        ...
        /// <param name="flowID">流程ID</param>
        /// <returns></returns>
        public RoadFlow.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(Guid flowID, bool cache = false)
        {
            if (!cache)
            {
        ...
        public string SaveFromData(string instanceid, RoadFlow.Data.Model.WorkFlowCustomEventParams eventParams)
        {
            ...
            System.Data.DataTable schemaDt = executeBLL.GetTableSchema(dbtable);
            //System.Data.IDataAdapter dataAdapter = executeBLL.GetDataAdapter(sql, parList.ToArray());
            System.Data.DataSet ds = new System.Data.DataSet();
            //dataAdapter.Fill(ds);
            System.Data.DataTable dt = executeBLL.GetDataTable(sql, parList.ToArray());
            #endregion
            ...
            if (isNew)
            {
                if (InsertSQL(dbtable, dt, out instanceid, dbtablepk, isNew, schemaDt, executeBLL) == 0)
                {
                    return instanceid;
                }
            }
            ...
            {
                if (UpdateSQL(dbtable, dt, instanceid, dbtablepk, isNew, schemaDt, executeBLL) == 0)
                {
                    return instanceid;
                }
            }
            ...
            #region 如果是新增，又是自增列则查询刚插入的自增列值,插入数据成功
            //if (isNew && isInt) {
            //    string identitysql = "SELECT @@IDENTITY";
            //    System.Data.DataTable obj = executeBLL.GetDataTable(identitysql);
            //    if (obj.Rows.Count > 0) {
            //        instanceid = obj.Rows[0][0].ToString();
        ...
        //SaveFromData 的子方法
        private int InsertSQL(string dbtable, System.Data.DataTable dt, out string instanceid, string dbtablepk, bool isNew, System.Data.DataTable schemaDt, ExecuteBLL executeBLL)
        {
            instanceid = null;
            ...
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                ...
                if (isSet)
                {
                    columnBuilder.Append(",[").Append(colnum.ColumnName).Append("]");
                    parameters.Add(new SqlParameter("@" + colnum.ColumnName, value));
                    valuesName.Append(",@").Append(colnum.ColumnName);
                }
            }
            if (columnBuilder.ToString().IsNullOrEmpty())
            {//没有添加任何列
                return 0;
            }
            else
            {
                ...
                columnBuilder.Append(",[").Append("ID").Append("]");
                parameters.Add(new SqlParameter("@ID", instanceid));
                valuesName.Append(",@").Append("ID");

                columnBuilder.Remove(0, 1);//移除首个"，"
                valuesName.Remove(0, 1);//移除首个"，"
                stringBuilder.AppendFormat("insert into [{0}]({1}) values({2})", dbtable, columnBuilder.ToString(), valuesName.ToString());
            }
            return executeBLL.ExecuteSQL(stringBuilder.ToString(), parameters.ToArray());
        }

        //SaveFromData 的子方法
        private int UpdateSQL(string dbtable, System.Data.DataTable dt, string instanceid, string dbtablepk, bool isNew, System.Data.DataTable schemaDt, ExecuteBLL executeBLL)
        {
            List<SqlParameter> parameters = new List<SqlParameter>();
            StringBuilder stringBuilder = new StringBuilder();
            StringBuilder valuesName = new StringBuilder();
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                ...
                {
                    parameters.Add(new SqlParameter("@" + colnum.ColumnName, value));
                    valuesName.Append(",[").Append(colnum.ColumnName).Append("]=@").Append(colnum.ColumnName);
                }
            }
            if (valuesName.ToString().IsNullOrEmpty())
            {//没有添加任何列
                return 0;
            }
            else
            {
                valuesName.Remove(0, 1);//移除首个"，"
                string where2 = "where [ID]='" + instanceid + "'";
                stringBuilder.AppendFormat("update [{0}] set {1} {2}", dbtable, valuesName.ToString(), where2);
            }
        ...
        //                case "MySql":
        //                    hasDefault = schemaDrs.Length > 0 && !schemaDrs[0]["cdefault"].ToString().IsNullOrEmpty();
        //                    hasNull =  schemaDrs[0]["is_null"].ToString() == "YES";
        //                    break;
        ...
        //                        if (colnumDataType == "System.DateTime")
        //                        {
        //                            dt.Rows[0][colnumName] = DateTime.Now.ToString();
        //                        }
        //                    }
        //                }
        //            }
        ...
        //                            break;
        //                        case "MySql":
        //                            isIdentity = schemaDrs[0]["isidentity"].ToString() != "PRI";
        //                            isDefault = !schemaDrs[0]["cdefault"].ToString().IsNullOrEmpty();
        //                            isGuid = !pkColnum.DataType.FullName.Contains("System.Int");
        //                            isInt = !isGuid;
        //                            break;
        //                    }
        ...
        //                break;
        //            case "MySql":
        //                MySql.Data.MySqlClient.MySqlCommandBuilder mysqlscb = new MySql.Data.MySqlClient.MySqlCommandBuilder((MySql.Data.MySqlClient.MySqlDataAdapter)dataAdapter);
        //                break;

        //        }
        //        try { dataAdapter.Update(ds); }
        //        catch (Exception e)
        //        {
        //            throw e;
        //        }
        ...
        //                    break;
        //                case "MySql":
        //                    string mysql = "SELECT @@IDENTITY ";
        //                        using (MySqlCommand mycmd = new MySqlCommand(mysql, (MySqlConnection)conn))
        //                        {
        ...
        //                    break;
        //                case "MySql":
        //                    MySql.Data.MySqlClient.MySqlCommandBuilder scb2 = new MySql.Data.MySqlClient.MySqlCommandBuilder((MySql.Data.MySqlClient.MySqlDataAdapter)dataAdapter1);
        //                    break;
