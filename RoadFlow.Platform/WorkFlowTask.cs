    ...
    public class WorkFlowTask : IEqualityComparer<RoadFlow.Data.Model.WorkFlowTask> {
        ...
        private void executeSubmit(RoadFlow.Data.Model.WorkFlowExecute.Execute executeModel) {
            ...
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope()) {
                ...
                eventParams.FlowID = executeModel.FlowID;
                eventParams.GroupID = executeModel.GroupID;
                eventParams.StepID = executeModel.StepID;
                eventParams.TaskID = executeModel.TaskID;
                eventParams.InstanceID = executeModel.InstanceID;
                //保存数据
                executeModel.InstanceID = new RoadFlow.Platform.WorkFlow().SaveFromData(executeModel.InstanceID, eventParams);
                result.InstanceID = executeModel.InstanceID;
            ...
            if (nextTasks.Count > 0) {
                ...
                //by hds 2016-3-3 10:15:10 发送短信
                List<string> nextStepName = new List<string>();//not by hds
                List<string> useridList = new List<string>();
                foreach (var nstep in nextTasks) {
                    nextStepName.Add(nstep.StepName);//not by hds
                ...
                tells = tells.Trim(',');
                if (!tells.IsNullOrEmpty()) {
                    SMSMessage message = new SMSMessage {
                        Mobile = tells,
                        Content = "【青羊楼宇】" + nextTasks[0].SenderName + "提交了“" + nextTasks[0].Title + "”信息，请及时处理。"
                    };
                    if (executeModel.FlowID != Guid.Parse("ECD1CEE2-D5DA-46CE-ADBA-DDCD3A885F8F") && executeModel.FlowID != Guid.Parse("2CED6604-420F-49E5-B93C-FD9D4E135E65") && executeModel.FlowID != Guid.Parse("3EAC851A-E6A3-4C42-9777-52B75EBCA459"))
                    {
                        new RoadFlow.Utility.SMS().SendSMS(message);
                    }
                }
                //end
        ...
        private void executeBack(RoadFlow.Data.Model.WorkFlowExecute.Execute executeModel) {
            ...
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope()) {
                ...
                foreach (var backTask in backTasks) {
                    if (backTask.Status.In(2, 3, 4, 5))//已完成的任务不能退回(2016.5.19)
                    {
                        continue;
                    }
            ...
            if (nextTasks.Count > 0) {
                ...
                if (!tells.IsNullOrEmpty()) {
                    SMSMessage message = new SMSMessage {
                        Mobile = tells,
                        Content = "【青羊楼宇】" + nextTasks[0].SenderName + "退回了“" + nextTasks[0].Title + "”信息，请及时处理。"
                    };
                    if (executeModel.FlowID != Guid.Parse("ECD1CEE2-D5DA-46CE-ADBA-DDCD3A885F8F") && executeModel.FlowID != Guid.Parse("2CED6604-420F-49E5-B93C-FD9D4E135E65") && executeModel.FlowID != Guid.Parse("3EAC851A-E6A3-4C42-9777-52B75EBCA459"))
                    {
                        new RoadFlow.Utility.SMS().SendSMS(message);
        ...
        private void executeComplete(RoadFlow.Data.Model.WorkFlowExecute.Execute executeModel, bool isCompleteTask = true) {
            ...
            if (task == null) {
                ...
                return;
            } else if (isCompleteTask && task.Status.In(2, 3, 4, 5)) {
                result.DebugMessages = "当前任务已处理";
                result.IsSuccess = false;
                result.Messages = "当前任务已处理";
                return;
            ...
            if (wfInstalled.TitleField != null && wfInstalled.TitleField.LinkID != Guid.Empty && !wfInstalled.TitleField.Table.IsNullOrEmpty()
                ...
                #region 假如有完成的后续处理方法，则调用
                Type t = Type.GetType("RoadFlow.Platform." + wfInstalled.TitleField.Table + "BLL,RoadFlow.Platform, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null");
                System.Reflection.MethodInfo methodInfo = t.GetMethod("Complete");
                if (methodInfo!=null) {
                    ConstructorInfo constructor = t.GetConstructor(Type.EmptyTypes);
                    object classObject = constructor.Invoke(new object[] { });
                    methodInfo.Invoke(classObject, new object[] { Guid.Parse(executeModel.InstanceID) });
                }
                #endregion
            }
            #endregion
        ...
        private RoadFlow.Data.Model.WorkFlowTask createFirstTask(RoadFlow.Data.Model.WorkFlowExecute.Execute executeModel, bool isSubFlow = false) {
            ...
            Add(task);
            #region 如果有前置，执行前置（如更新楼栋时需要将流程状态（state字段）重置为0）
            if (!string.IsNullOrWhiteSpace(executeModel.InstanceID)) {
                Type t = Type.GetType("RoadFlow.Platform." + wfInstalled.TitleField.Table + "BLL,RoadFlow.Platform, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null");
                if(t==null)
                {
                    //历史使用的非BLL业务逻辑处理。
                ...
                {
                    System.Reflection.MethodInfo methodInfo = t.GetMethod("Begin");
                    if (methodInfo != null) {
                        ConstructorInfo constructor = t.GetConstructor(Type.EmptyTypes);
                        object classObject = constructor.Invoke(new object[] { });
                        methodInfo.Invoke(classObject, new object[] { executeModel.InstanceID });
                    }    
                }
        ...
        /// <returns></returns>
        public List<RoadFlow.Data.Model.WorkFlowTask> GetTaskList(Guid taskID) {
        ...
        public bool WithdrawTask(Guid taskID) {
            var taskList1 = Get(taskID);
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope()) {
                var taskList2 = GetNextTaskList(taskList1.ID);
                foreach (var task2 in taskList2) {
                    Delete(task2.ID);
                }
                if (taskList1.ID == taskID) {
                    Completed(taskList1.ID, "", false, 1, "");
                }
                scope.Complete();
                return true;
            }
        ...
        /// <param name="groupID"></param>
        /// <returns></returns>
        public List<RoadFlow.Data.Model.WorkFlowTask> GetTaskListByInstance(Guid instanceID) {
            return dataWorkFlowTask.GetTaskListByInstance(instanceID);
        }
