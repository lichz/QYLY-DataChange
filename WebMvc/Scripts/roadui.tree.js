...
; RoadUI.Tree = function (options) {
    ...
    this.addNodes = function (json, $node, isRefresh, isInit) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            var tree_div_style = "";
            if (this.opts.isnew) {
                tree_div_style = ' style="padding: 4px;"';
            }

            var $div = $('<div id="roadui_tree_' + id + '" class="tree_div" ' + tree_div_style + '></div>');

            var tree_ul_new = "";
            if (this.opts.isnew) {
                tree_ul_new = 'style="height:22px; line-height:16px;"';
            }

            var $ul = $('<ul class="tree_ul" ' + tree_ul_new + '></ul>');
            $div.data("json", jsoni);
            if (isroot) {
                $div.attr("isroot", "1");
            ...
            if (this.opts.showico || isroot) {
                ...
                if (this.opts.isnew) {
                    $ico = $('<li class="tree_li"><span class="' + icoClass + '" style="background:url(/Content/Theme/Blue/images/tree/tree_blue.png)  no-repeat; width:26px; " ></span></li>');
                }
            ...
            var tree_title_new = "";
            if (this.opts.isnew) {
                tree_title_new = 'style="font-size: 14px;"';
            }

            var $title = $('<li class="tree_li"><span class="tree_title" ' + tree_title_new + '>' + title + '</span></li>');
            $ul.append($title);
            ...
            var tree_ul_over_new = "tree_ul_over";
            if (this.opts.isnew) {
                tree_ul_over_new = "tree_ul_over_new";
            }

            $ul.bind("mouseover", function () {
                $(this).removeClass().addClass(tree_ul_over_new);
