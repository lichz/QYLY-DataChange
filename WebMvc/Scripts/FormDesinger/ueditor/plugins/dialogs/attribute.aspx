...
<table cellpadding="0" cellspacing="1" border="0" width="95%" class="formtable">
    ...
    <tr>
        <th>数据连接:</th>
        <td><select class="myselect" id="dbconn" onchange="db_change($(this).val())" style="width:227px"><%=link_DBConnOptions %></select></td>
    </tr>
    <tr>
        <th>数据表:</th>
        <td><select class="myselect" id="dbtable" onchange="table_change($(this).val())" style="width:227px"></select></td>
    </tr>
    <tr>
        <th>主键:</th>
...
<script type="text/javascript">
    ...
    {  
        ...
        {
            db_change($("#dbconn").get(0).value, "");
        }
    });
    function db_change(conn, table)
    {
        if (conn == "" || conn=="undefind") return;
        $("#dbtable").html(getTableOps(conn, table));
  
        table_change($("#dbtable").val(), dbtablepk);
    }
    function table_change(dbtable, fields)
    {
        if (dbtable == "" || dbtable == "undefind") return;
        var conn = dbconn;
        console.log(dbtable + conn);
        var opts = getFieldsOps(conn, dbtable, fields);
        $("#dbpk").html(opts);
        if (!isnew)
        {
            var opts1 = getFieldsOps(conn, dbtable, dbtabletitle);
            $("#dbtitle").html(opts1);
        }
        else
        {
